...
class CreateTerryStrategyKnowledge extends Migration {
	 ...
	 */
	public function up() {
		Schema::create('terry_strategy_knowledge', function(Blueprint $table) {
			$table->increments('id');
			...
			$table->decimal('long_wins_per_day', 30, 5)->index('long_wins_per_day1');
			$table->decimal('short_wins_per_day', 30, 5)->index('short_wins_per_day1');
			$table->decimal('long_loses_per_day', 30, 5)->index('long_loses_per_day1');
			$table->decimal('short_loses_per_day', 30, 5)->index('short_loses_per_day1');
			$table->decimal('timeout_loses_per_day', 30, 5)->index('timeout_loses_per_day1');
			$table->decimal('longs_per_day', 30, 5)->index('longs_per_day1');
