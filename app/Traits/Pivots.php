<?php

/**
 * Created by PhpStorm.
 * User: joeldg
 * Date: 7/4/17
 * Time: 10:46 AM
 */

namespace Bowhead\Traits;

use Bowhead\Traits\OHLC;

/**
 * Class Pivots
 * @package Bowhead\Util
 *          using algorithms from:
 *          http://stockcharts.com/school/doku.php?id=chart_school%3Atechnical_indicators%3Apivot_points
 */
trait Pivots {

	function calc_fib($data, $period = 15) {
		if (empty($data)) {
			$data = $this->getRecentData('BTC/USD');
		}
		/**
		  Pivot Point   (P) = (High + Low + Close)/3
		  Support 1    (S1) = P - {.382 * (High  -  Low)}
		  Support 2    (S2) = P - {.618 * (High  -  Low)}
		  Support 3    (S3) = P - {1 * (High  -  Low)}
		  Resistance 1 (R1) = P + {.382 * (High  -  Low)}
		  Resistance 2 (R2) = P + {.618 * (High  -  Low)}
		  Resistance 3 (R3) = P + {1 * (High  -  Low)}
		  // */
		$data['high'] = array_values($data['high']);
		$data['low'] = array_values($data['low']);
		$data['close'] = array_values($data['close']);
		$num = count($data['high']) - (round($period / 2) * 2);

		if ($num < 0) {
			return FALSE; 
		}

		$ret = [];
		for ($a = $num; $a < count($data['high']); $a++) {
			$p = ($data['high'][$a] + $data['low'][$a] + $data['close'][$a]) / 3;
			$s1 = $p - (.382 * ($data['high'][$a] - $data['low'][$a]));
			$s2 = $p - (.618 * ($data['high'][$a] - $data['low'][$a]));
			$s3 = $p - (1 * ($data['high'][$a] - $data['low'][$a]));
			$r1 = $p + (.382 * ($data['high'][$a] - $data['low'][$a]));
			$r2 = $p + (.618 * ($data['high'][$a] - $data['low'][$a]));
			$r3 = $p + (1 * ($data['high'][$a] - $data['low'][$a]));

			/** get the average for the prior period */
			$ret['P'] = ((@$ret['P'] + $p)/2) ?? $p;
			$ret['S1'] = ((@$ret['S1'] + $s1)/2) ?? $s1;
			$ret['S2'] = ((@$ret['S2'] + $s2)/2) ?? $s2;
			$ret['S3'] = ((@$ret['S3'] + $s3)/2) ?? $s3;
			$ret['R1'] = ((@$ret['R1'] + $r1)/2) ?? $r1;
			$ret['R2'] = ((@$ret['R2'] + $r2)/2) ?? $r2;
			$ret['R3'] = ((@$ret['R3'] + $r3)/2) ?? $r3;
		}
		return $ret;
	}

	function calc_demark($data, $period = 120) {
		if (empty($data)) {
			$data = $this->getRecentData('BTC/USD');
		}
		/**
		  If Close < Open, then X = High + (2 x Low) + Close
		  If Close > Open, then X = (2 x High) + Low + Close
		  If Close = Open, then X = High + Low + (2 x Close)
		  Pivot Point (P) = X/4
		  Support 1 (S1) = X/2 - High
		  Resistance 1 (R1) = X/2 - Low
		  // */
		$data['open'] = array_values($data['open']);
		$data['high'] = array_values($data['high']);
		$data['low'] = array_values($data['low']);
		$data['close'] = array_values($data['close']);
		$num = count($data['high']) - (round($period / 2) * 2);

		if ($num < 0) {
			return FALSE; 
		}

		$ret = [];
		for ($a = $num; $a < count($data['high']); $a++) {
			if ($data['close'][$a] < $data['open'][$a]) {
				$x = ($data['high'][$a] + ($data['low'][$a] * 2) + $data['close'][$a]);
			}
			if ($data['close'][$a] > $data['open'][$a]) {
				$x = (($data['high'][$a] * 2) + $data['low'][$a] + $data['close'][$a]);
			}
			if ($data['close'][$a] == $data['open'][$a]) {
				$x = ($data['high'][$a] + $data['low'][$a] + ($data['close'][$a] * 2));
			}
			$p = $x / 4;
			$s1 = $x / 2 - $data['high'][$a];
			$r1 = $x / 2 - $data['low'][$a];

			$ret['P'] = ((@$ret['P'] + $p)/2) ?? $p;
			$ret['S1'] = ((@$ret['S1'] + $s1)/2) ?? $s1;
			$ret['R1'] = ((@$ret['R1'] + $r1)/2) ?? $r1;
		}
		return $ret;
	}

	function calc_pivot($data, $period = 120) {
		if (empty($data)) {
			$data = $this->getRecentData('BTC/USD');
		}
		/**
		  Pivot Point (P) = (High + Low + Close)/3
		  Support 1 (S1) = (P x 2) - High
		  Support 2 (S2) = P  -  (High  -  Low)
		  Resistance 1 (R1) = (P x 2) - Low
		  Resistance 2 (R2) = P + (High  -  Low)
		  // */
		$data['high'] = array_values($data['high']);
		$data['low'] = array_values($data['low']);
		$data['close'] = array_values($data['close']);
		$num = count($data['high']) - (round($period / 2) * 2);

		if ($num < 0) {
			return FALSE; 
		}

		$ret = [];
		for ($a = $num; $a < count($data['high']); $a++) {
			$p = ($data['high'][$a] + $data['low'][$a] + $data['close'][$a]) / 3;
			$s1 = ($p * 2) - $data['high'][$a];
			$s2 = ($p - ($data['high'][$a] - $data['low'][$a]));
			$r1 = ($p * 2) - $data['low'][$a];
			$r2 = ($p + ($data['high'][$a] - $data['low'][$a]));

			$ret['P'] = ((@$ret['P'] + $p)/2) ?? $p;
			$ret['S1'] = ((@$ret['S1'] + $s1)/2) ?? $s1;
			$ret['S2'] = ((@$ret['S2'] + $s2)/2) ?? $s2;
			$ret['R1'] = ((@$ret['R1'] + $r1)/2) ?? $r1;
			$ret['R2'] = ((@$ret['R2'] + $r2)/2) ?? $r2;
		}
		return $ret;
	}

	/**
	 * See if one of the pivot boundaries (demark, fib) was crossed recently
	 */
	function get_crossed_pivot_boundaries($data = null, $numperiods = 5) {
		$fibs = $this->calc_fib($data);
		unset($fibs['P']);
		$demark = $this->calc_demark($data);
		unset($demark['P']);
		$pivot = $this->calc_pivot($data);
		unset($pivot['P']);

		$boundary_values = [];
		if(is_array($fibs)) {
			foreach ($fibs as $k => $v) {
				$boundary_values['fib_' . $k] = $v;
			}
		}
		if(is_array($demark)) {
			foreach ($demark as $k => $v) {
				$boundary_values['demark_' . $k] = $v;
			}
		}
		if(is_array($pivot)) {
			foreach ($pivot as $k => $v) {
				$boundary_values['pivot_' . $k] = $v;
			}
		}

		$len = count($data['close']);
		
		if($len < $numperiods) {
			return FALSE;
		}

		$boundaries_crossed = [];
		foreach ($boundary_values as $boundary => $value) {
			$above = $below = FALSE;
			foreach (['open', 'high', 'low', 'close'] as $fig) {
				for ($a = $len - $numperiods; $a < $len; $a++) {
					if ($data[$fig][$a] >= $value) {
						$above = $above || TRUE;
					}
					if ($data[$fig][$a] <= $value) {
						$below = $below || TRUE;
					}
				}
			}
			$boundaries_crossed[$boundary] = $above && $below;
		}
		return $boundaries_crossed;
	}

}
